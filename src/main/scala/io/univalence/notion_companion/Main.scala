...
sealed trait Notion {
  ...
  def database(databaseId: DatabaseId, filter: Option[NotionDatabaseFilter], sort: Option[NotionDatabaseSort]): Task[JValue]
...
object Notion extends Accessible[Notion] {
  val live: URLayer[Configuration with SttpClient, LiveNotionAPI] = ZLayer {
    for {
...
case class LiveNotionAPI(config: NotionConfiguration, sttpClient: SttpClient) extends Notion {
  ...
  private def defaultRequest = basicRequest
    .auth
  ...
  override def database(databaseId: DatabaseId,
...
object Main extends ZIOAppDefault {
  ...
  def app: ZIO[Notion, Nothing, Unit] = for {
